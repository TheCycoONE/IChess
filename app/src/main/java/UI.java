...
public class UI extends JFrame implements ActionListener {
  ...
  public synchronized int chosenSquare(Point cSquare) {
    ...
    if (origin != null) {
      if ((origin.x + origin.y) % 2 == 1) {
        bp.highlight(origin, Color.WHITE);
      } else {
        bp.highlight(origin, Color.BLACK);
      }
    }
    ...
    while (flag) {
      ...
      switch (endGame.getOption()) {
        ...
        case EndGameDialog.PLAY_AGAIN:
          flag = false;
          resetGame(Board.createInitialBoard());
          break;
  ...
  public synchronized void defaultBoard() {
    ...
    theBoard = Board.createInitialBoard();
    resetGame(theBoard);
  }
