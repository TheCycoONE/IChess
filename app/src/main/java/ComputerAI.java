  ...
  private int alphaBetaEval(Board node, int alpha, int beta, int ft) {
    ...
    if (ft % 2 == 1) // Minimizing (Black)
    {
      ...
      for (iPiece = 0; iPiece < node.blackPieces.size(); iPiece++) {
        feasible = action.getFeasibleMoves(node, node.blackPieces.elementAt(iPiece));

        for (iFeasible = 0; iFeasible < feasible.size(); iFeasible++) {
          mate = false; // there is at least one feasible move
          moves =
              action.move(node, node.blackPieces.elementAt(iPiece), feasible.elementAt(iFeasible));
    ...
    } else // Maximizing (White)
    {
      ...
      for (iPiece = 0; iPiece < node.whitePieces.size(); iPiece++) {
        feasible = action.getFeasibleMoves(node, node.whitePieces.elementAt(iPiece));

        for (iFeasible = 0; iFeasible < feasible.size(); iFeasible++) {
          mate = false; // there is at least one feasible move
          moves =
              action.move(node, node.whitePieces.elementAt(iPiece), feasible.elementAt(iFeasible));
          for (iMove = 0; iMove < moves.size(); iMove++) {
            tmpBoard = moves.elementAt(iMove);
    ...
    for (i = 0; i < aBoard.whitePieces.size(); i++) {
      ...
      switch (piece) {
        case 1: // Pawn
          ...
          if ((location.x >= aBoard.theKings[0].x - 1 && location.x <= aBoard.theKings[0].x + 1)
    ...
    for (i = 0; i < aBoard.blackPieces.size(); i++) {
      ...
      switch (piece) {
        case -1: // Pawn
          ...
          // Pawns should not block king's movement
          if ((location.x >= aBoard.theKings[1].x - 1 && location.x <= aBoard.theKings[1].x + 1)
